...
async function main() {
    try {
        ...
        const [rows] = await connection.execute('SHOW PROCESSLIST') as any[];
        console.log('Current DB Processes:');
        rows.forEach((r: any) => {
            // Filter out my own connection and sleep
            if (r.Command !== 'Sleep' && r.Info !== 'SHOW PROCESSLIST') {
                console.log(`[${r.Id}] User:${r.User} | Host:${r.Host} | DB:${r.db} | Cmd:${r.Command} | Time:${r.Time} | State:${r.State} | Info:${r.Info ? r.Info.substring(0, 100) : 'NULL'}`);
            }
        });
