...
import { extractDataFromContent as extractDataDeepSeek } from './deepseekService';
import { extractTextFromImageAliyun } from './aliyunService';

/**
 * 主解析入口
 * 流程：文件 -> 文本提取 (OCR/Mammoth/XLSX) -> DeepSeek 解析 -> 结构化数据
 */
/**
 * 主解析入口
...
export const parseInquiryFile = async (filePath: string, fileName: string, onProgress?: (data: { rawContent?: any, logs?: any[] }) => Promise<void>) => {
    ...
    const logs: any[] = [];
    const addLog = (step: string, message: string, status: 'success' | 'info' | 'error' = 'info') => {
        const log = { time: new Date().toISOString(), step, message, status };
        logs.push(log);
    ...
    try {
        ...
        if (ext === '.xlsx' || ext === '.xls') {
            ...
            const fileData = fs.readFileSync(filePath);
            const base64Data = fileData.toString('base64');
            addLog('PDF解析', `正在通过 OCR 解析 PDF 内容...`, 'info');
            extractedText = await extractTextFromImageAliyun({ mimeType: 'application/pdf', data: base64Data });
            extractedData = extractedText.split('\n').filter(l => l.trim()).map(l => ({ text: l }));
            addLog('PDF解析', `PDF 解析成功`, 'success');
        ...
        // 调用 DeepSeek 进行深度解析
        addLog('AI分析', `正在通过 DeepSeek 模型进行智能匹配与标准化...`, 'info');
        const parsedResult = await extractDataDeepSeek(extractedText);
