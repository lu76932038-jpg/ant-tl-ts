...
const API_URL = 'http://101.132.103.175:7900/ant-tool/api/community/questions';

// 专用于获取数据的自动登录账号信息（已在后端数据库中预置）
const BOT_CREDENTIALS = {
    username: 'admin',
    password: 'admin123'
};
// 为了避免并发或频繁写入，保存一个游标状态文件
const STATE_FILE = path.join(__dirname, '..', '..', '.agent', 'tmp', 'sync_community_state.json');
const OUTPUT_FILE = path.join(__dirname, '..', '..', '.agent', 'tmp', 'new_community_questions.json');
...
// 追加保存新的帖子数据
function saveNewQuestions(questions: any[]) {
    ...
    const newItems = questions.filter((q: any) => !existingIds.has(q.id));

    const updatedList = [...existing, ...newItems];
    fs.writeFileSync(OUTPUT_FILE, JSON.stringify(updatedList, null, 2));
    console.log(`✅ [本地存储] 成功新增 ${newItems.length} 条社区帖子，当前未处理列表剩余 ${updatedList.length} 条。`);
}

async function main() {
    console.log(`🚀 [任务启动] 开始抓取社区问答帖子。执行模式: ${isManual ? '手工触发' : '定时执行'}`);
    ...
    try {
        ...
        const questions = Array.isArray(responseData.data) ? responseData.data : (Array.isArray(responseData) ? responseData : []);

        if (questions.length === 0) {
            console.log(`ℹ️ [结果] 没有检测到新的社区帖子数据。`);
        } else {
            console.log(`🎉 [结果] 成功获取到 ${questions.length} 条新社区帖子!`);
            saveNewQuestions(questions);
        }
        ...
        console.error(`❌ [错误] 获取社区帖子失败:`, error.message);
        if (error.response) {
            console.error(`❌ 后端响应状态: ${error.response.status}`);
            if (error.response.status === 404) {
                console.error(`⚠️ 前方提示: 请确认后端服务代码中是否已经编写了 /community/questions 路由接口。`);
            }
        }
    }
}
