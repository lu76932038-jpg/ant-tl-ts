        ...
        children: [
            ...
            },
            {
                ...
                children: [
                    ...
                    { label: '对话审计日志', path: '/rag/logs', icon: <History className="w-4 h-4" />, roles: ['admin'], permission: 'rag_logs' },
                    { label: 'RAG 训练管理', path: '/rag/training', icon: <Settings className="w-4 h-4" />, roles: ['admin'], permission: 'rag_training' }
                ]
            },
            {
                label: '备货助手',
                ...
                children: [
                    ...
                    { label: '操作指引', path: '/stock/help', icon: <BookOpen className="w-4 h-4" />, permission: 'stock_help' },
                ]
            },
            {
                label: '报表管理',
                ...
                children: [
                    { label: '智能报表', path: '/report-management', icon: <FileText className="w-4 h-4" />, roles: ['user', 'admin'], permission: 'report_management' }
                ]
            },
            {
                label: '客户管理',
    ...
    },
    {
        ...
        children: [
            { label: '用户管理', path: '/users', icon: <Users className="w-4 h-4" />, roles: ['admin'], permission: 'user_management' },
            {
                ...
                roles: ['admin'],
                children: [
                    { label: '出库数据同步', path: '/system/data-sync/outbound', icon: <Truck className="w-4 h-4" />, roles: ['admin'], permission: 'data_sync' },
                    { label: '出库计划同步', path: '/system/data-sync/outbound-plan', icon: <Truck className="w-4 h-4" />, roles: ['admin'], permission: 'data_sync' },
                    { label: '库存数据同步', path: '/system/data-sync/inventory', icon: <Database className="w-4 h-4" />, roles: ['admin'], permission: 'data_sync' },
                    { label: '入库数据同步', path: '/system/data-sync/inbound', icon: <Database className="w-4 h-4" />, roles: ['admin'], permission: 'data_sync' }
                ]
            },
            { label: '登录日志', path: '/login-logs', icon: <History className="w-4 h-4" />, roles: ['admin'], permission: 'login_logs' }
        ]
    }
];
