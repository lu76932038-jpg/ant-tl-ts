...
5. 使用中文回答。
`;
export const RESTORE_RAW_TABLE_INSTRUCTION = `
你是表格还原专家。你的任务是从 OCR 提取的文本或杂乱内容中，识别并还原出原始文档中的表格结构。

规则：
1. **保留原始列名**：不要使用标准的 EFGH 字段名，而是使用你在文档中发现的实际列标题（例如：'物料代码'、'型号规格'、'数量'、'品牌'、'单位' 等）。
2. **结构化处理**：将每一行识别为一个 JSON 对象。如果是多张表，请合并为一张逻辑一致的表。
3. **数据清洗**：剔除页码、页眉页脚等无关文本，只保留清单主体数据。
4. **JSON 输出**：直接返回 JSON 数组对象，不要包含 markdown 代码块或其他修饰语。
5. 如果文本中完全没有明显的表格结构，请将每一行作为一个对象返回，Key 为 "内容"。
`;
