...
const OutboundPlanSync: React.FC = () => {
    const [config, setConfig] = useState<SyncConfig>({
        ...
        password: '',
        database: '',
        sql: 'SELECT plan_code, product_model, product_name, quantity, customer_name, customer_code, planned_date, warehouse FROM view_outbound_plan WHERE planned_date >= NOW()',
        schedule: ['03:00']
    });
    ...
    const fetchStatus = async () => {
        try {
            ...
            setIsSyncing(res?.isOutboundPlanSyncing ?? false);
        } catch (error) {
    ...
    const fetchConfig = async () => {
        try {
            const data: any = await api.get('/datasync/config/outbound-plan');
            ...
            console.error('Failed to fetch outbound plan config', error);
        }
    };

    const fetchLogs = async () => {
        try {
            const data: any = await api.get('/datasync/logs?type=outbound-plan');
            // Parse raw strings
    ...
    const saveDbConfig = async () => {
        try {
            ...
            await api.post('/datasync/config/outbound-plan', { host, port, user, password, database });
            alert('数据库配置已保存');
        } catch (error) {
            alert('保存失败');
        } finally {
    ...
    const saveStrategy = async () => {
        try {
            setIsLoading(true);
            await api.post('/datasync/config/outbound-plan', { sql: config.sql });
            alert('出库计划抽取策略已保存');
        } catch (error) {
            alert('保存失败');
        } finally {
    ...
    const saveSchedule = async () => {
        try {
            setIsLoading(true);
            await api.post('/datasync/config/outbound-plan', { schedule: config.schedule });
            alert('出库计划定时任务已保存');
        } catch (error) {
            alert('保存失败');
        } finally {
    ...
    const handleSync = async () => {
        if (!confirm('确定要立即执行出库计划同步吗？')) return;
        try {
            setIsLoading(true);
            await api.post('/datasync/sync/outbound-plan');
            alert('出库计划同步任务已启动，请查看日志进度。');
            fetchLogs();
        } catch (error) {
            alert('同步启动失败');
        } finally {
        ...
        <div className="flex flex-col h-full bg-[#f8f9fa] p-8 overflow-y-auto">
            <header className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">配置外部数据库并同步出库计划到系统</p>
                </div>
            </header>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input
                                    ...
                                    placeholder="erp_outbound_plan_db"
                                />
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 text-xs text-amber-800">
                                ...
                                <div className="flex flex-wrap gap-2 mt-2">
                                    <span className="bg-white border border-amber-200 px-2 py-0.5 rounded" title="必填"><b>plan_code</b>: 计划单号</span>
                                    ...
                                    <span className="bg-white border border-amber-200 px-2 py-0.5 rounded" title="必填"><b>quantity</b>: 计划数量</span>
                                    <span className="bg-white border border-amber-200 px-2 py-0.5 rounded" title="必填"><b>planned_date</b>: 计划日期</span>
                                    <span className="bg-white border border-amber-200 px-2 py-0.5 rounded" title="必填"><b>customer_name</b>: 客户名称</span>
                                    <span className="bg-white border border-blue-200 text-blue-800 px-2 py-0.5 rounded" title="选填"><b>customer_code</b>: 客户编码 (选填)</span>
                            ...
                            <textarea
                                ...
                                placeholder="SELECT code as plan_code, model as product_model..."
                            />
                        </div>
                    </div>
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 h-full flex flex-col sticky top-8 max-h-[calc(100vh-4rem)]">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 font-mono text-[11px] bg-slate-50 p-4 rounded-xl custom-scrollbar">
                            {logs.length === 0 ? (
                                <div className="text-center py-20 text-slate-400">暂无出库计划同步日志</div>
                            ) : (
                                logs.map((log, i) => (
...
export default OutboundPlanSync;
